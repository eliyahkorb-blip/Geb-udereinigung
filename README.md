...
- **Hinweis:** In der Entwicklungsumgebung, in der diese Änderung erstellt wurde, ist kein Zugriff auf das öffentliche Internet möglich (die Sandbox blockiert jegliche ausgehende HTTPS-Verbindung zu externen Domains, auch zu `github.io` selbst). Die echte, öffentliche GitHub-Pages-URL konnte daher nicht direkt per Browser/HTTP aufgerufen werden. Die obige lokale Simulation bildet die Pfadstruktur exakt nach und deckt damit genau die Fehlerklasse ab, die zuvor gemeldet wurde.

### Nachkontrolle: „Lädt auf manchen Geräten weiterhin ohne CSS/Bilder“

Nach dem ersten Fix wurde gemeldet, dass die Live-Seite auf manchen Geräten weiterhin ungestylt lädt. Da direkter HTTP-Zugriff auf `github.io` weiterhin nicht möglich ist, wurde stattdessen der **tatsächlich deployte Commit über die GitHub-API** kontrolliert (die GitHub-API läuft über einen separaten, nicht blockierten Zugang):

- `GET /repos/.../actions/runs` zeigt, dass GitHub Pages direkt vom Branch `claude/koenig-gebaeudereinigung-rebuild-8lc170` deployt (kein separater `main`/`gh-pages`-Branch, kein Actions-Workflow nötig) und dass der Workflow „pages build and deployment“ für den Fix-Commit bereits erfolgreich abgeschlossen war (`conclusion: success`).
- Der über die API abgerufene Quelltext von `index.html` und aller Asset-Ordner (`assets/css`, `assets/js`, `assets/icons`, `assets/img`) entspricht exakt dem lokal erzeugten, korrekten Stand: relative Pfade, keine Groß-/Kleinschreibungsfehler, keine falschen Dateiendungen (alle Bilder sind konsistent `.svg`, wie referenziert).
- Damit ist bestätigt: **der Code auf GitHub ist korrekt.** Die wahrscheinlichste Ursache für ein noch fehlerhaftes Laden auf einzelnen Geräten kurz nach dem Push ist eine Kombination aus:
  1. **Fehlender `.nojekyll`-Datei** — GitHub Pages verarbeitet Inhalte standardmäßig über Jekyll, was bei einer reinen statischen HTML/CSS/JS-Seite ohne Jekyll-Konfiguration zu unnötiger Verarbeitung und in Einzelfällen zu inkonsistentem Verhalten führen kann. Eine leere `.nojekyll`-Datei im Repo-Root wurde ergänzt, damit GitHub Pages die Dateien unverändert ausliefert.
  2. **CDN-/Browser-Caching** — GitHub Pages liefert über das Fastly-CDN aus; unmittelbar nach einem Push kann es je nach angefragtem Edge-Knoten und Browser-Cache einige Minuten dauern, bis überall die neue Version ausgeliefert wird. Geräte, die die Seite bereits vor dem Fix besucht haben, können zudem eine alte, zwischengespeicherte Version von `index.html` selbst anzeigen.
- Als Gegenmaßnahme wurden `assets/css/style.css` und `assets/js/main.js` in allen HTML-Dateien mit einem Versions-Query-Parameter versehen (`style.css?v=2`, `main.js?v=2`). Bei jeder künftigen CSS/JS-Änderung sollte `ASSET_VERSION` im Generator-Skript hochgezählt werden, damit Browser die neue Version garantiert nachladen statt eine gecachte Kopie zu verwenden.
- **Empfehlung für betroffene Geräte:** Hard-Reload durchführen (Strg/Cmd+Shift+R bzw. Browser-Cache leeren) oder wenige Minuten warten, bis die CDN-Auslieferung überall aktualisiert ist. Sollte das Problem danach fortbestehen, bitte die Browser-Konsole (F12 → Network/Console) auf konkrete 404-URLs prüfen und diese zurückmelden — anhand der URL lässt sich der Fehler dann gezielt eingrenzen.
